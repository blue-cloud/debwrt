...
static void cprint_init(void)
{
	...
	indent = 0;
	child_count = 0;
	cprint("./config/mconf/lxdialog/lxdialog");
	cprint("--backtitle");
...
int main(int ac, char **av)
{
	...
	sym = sym_lookup("DEBWRTVERSION", 0);
	sym_calc_value(sym);
	sprintf(menu_backtitle, _("DebWrt %s Configuration"),
		sym_get_string_value(sym));
	...
	do {
		cprint_init();
		cprint("--yesno");
		cprint(_("Do you wish to save your new DebWrt configuration?"));
		cprint("5");
		cprint("60");
		stat = exec_conf();
	} while (stat < 0);
	...
	if (stat == 0) {
		if (conf_write(NULL)) {
			fprintf(stderr, _("\n\n"
				"Error during writing of the DebWrt configuration.\n"
